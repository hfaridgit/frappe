...
frappe.views.QueryReport = Class.extend({
	...
	get_formatter: function() {
		...
		var formatter = function(row, cell, value, columnDef, dataContext, for_print) {
			...
			// @custom
			let value_with_tooltip = me.set_static_tooltips(row, cell, value, columnDef, dataContext, for_print);
			if (value_with_tooltip !== false) value = value_with_tooltip;
	...
	// @custom
	set_static_tooltips: function(row, cell, value, columnDef, dataContext, for_print) {
		let query_report_opts = this.get_query_report_opts();
		var static_tooltip = query_report_opts.static_tooltip;

		if (static_tooltip) {
			var tooltip = '';

			if (static_tooltip.fieldname) {
				let fieldtype = 'Data';

				if (!dataContext[static_tooltip.fieldname]) return false;
				if (static_tooltip.label) tooltip += frappe._(static_tooltip.label) + ': ';
				tooltip += frappe.format(dataContext[static_tooltip.fieldname], {fieldtype: static_tooltip.fieldtype});
